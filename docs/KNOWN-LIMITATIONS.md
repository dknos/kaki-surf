...
- **Wave coverage:** Golden Coast, Twilight Glass, Stormbreak, and Kaki-Land share the production long-face/gravity-column compositor while retaining condition-owned physics, palettes, and encounter pacing. Kaki-Land adds the authored `signalBreak` seam and network presentation without a parallel collision surface. More condition-specific silhouettes remain future work; the retired mini-curl, white shelf, and full-image C-wave renderers are no longer selected by any condition.
...
- **Options scope:** remapping, text-size controls, and automatic initialization from the operating system's Reduced Motion preference remain unimplemented. Existing persistent controls cover Auto/Full/Mobile visual quality, Simple/Advanced mode, audio, shake, flash, contrast, touch permission, wave-read guidance, and steering/landing assists.
...
- **Low-powered hardware coverage:** Auto quality now selects a presentation-only Mobile profile on compact touch devices and adapts after sustained missed frames. The fixed-step cap, bounded pools, reduced-motion path, and compact Web Audio graph are implemented, but extended thermal, battery, audio-latency, and frame-pacing tests on older physical phones, integrated GPUs, and power-saving browser modes remain outstanding.
...
5. Add the missing remap/text-size/OS-preference options and profile long Mobile-quality runs on representative low-powered physical devices.

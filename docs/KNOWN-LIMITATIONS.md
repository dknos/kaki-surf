...
The implementation includes Simple and Advanced controls, compositional aerials, bidirectional/switch surfing, three boards and conditions, a seeded wildlife/traffic/powerup world, local generated atlases with fallbacks, access settings, a synchronized 118-state browser QA matrix, and a static-host adapter. The current release is a strong score-attack slice, but the broader commercial-arcade roadmap still has product work as well as physical-lab validation.
...
- **Wave coverage:** Twilight Glass owns the finished hero-barrel profile and is the fresh-player default. Golden Coast and Stormbreak still use the earlier classic profile; their distinct authored level waves remain future work.
...
2. Author the next level-specific wave profile only after Twilight remains visually and mechanically locked.

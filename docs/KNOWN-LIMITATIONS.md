...
- **Wave coverage:** Golden Coast, Twilight Glass, and Stormbreak now share the production continuous long-break compositor while retaining different surface physics, palettes, and encounter pacing. Distinct authored silhouettes for the next two levels remain future work; the retired mini-curl/white-shelf renderer is no longer selected by any condition.
...
- **Fallback fidelity:** a missing continuous-wave or travelling-break atlas retains the complete collision-aligned one-path wall, tube, trough, downward curtain, and churn, but loses the selected Grok face texture and foam density.

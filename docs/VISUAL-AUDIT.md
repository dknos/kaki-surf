...
- the selected one-frame 352 x 198 long-barrel atlas supplies the stable normal-palette back wall, broad D-shaped pocket, and curved trough planes. It renders behind Kaki; a segmented falling edge, low foam, and spray render in front, with the coherent-break and earlier full-barrel studies retained in the fallback chain;
...
All selected Grok source sheets and all 17 output atlases were inspected at actual size during authoring, including the 1024 x 1024 modular wave source and the 1280 x 720 staged-breaker, long-barrel back, earlier full-barrel, component, waterfall-curtain, and coherent-break sources.
...
- Normal Twilight rendering uses the long-barrel back, Kaki, segmented falling veil/foreground foam, and component `contactSpray` in that order. The back layer's stepped left alpha opens cleanly onto real sky without an opaque source seam. High Contrast or a missing primary atlas uses the coherent break and earlier coherent curl before the complete procedural fallback; `foamCrown`, `faceRibbons`, and `foregroundShoulder` remain packed for provenance but are not rendered.
- Two local Qwen seam edits were compared at source and runtime size. The first cropped the crest and contaminated the chroma field; the native retry reintroduced long horizontal rails. Both remain documented rejected sources and are absent from the build.

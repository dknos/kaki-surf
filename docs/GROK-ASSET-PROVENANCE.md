...
The wave-breaker polish source is 1024 x 1024; the selected long Twilight back layer, earlier full Twilight barrel, staged wave progression, Twilight components, final curtain/break sheets, and other selected sources are 1280 x 720. SHA-256 values identify the exact files used at this checkpoint.

| Family | Selection | Preserved source | Source SHA-256 | Runtime atlas | Atlas dimensions |
| --- | --- | --- | --- | --- | ---: |
| Twilight long barrel back | Targeted edit v1 selected; original and v2 preserved; two local Qwen seam passes rejected | `docs/art-source/grok/twilight-long-barrel-back-edit-v1.png` | `269800eda215821d40c9bbf24eaf7fa77f47e3a2159e36b9221720892975e05b` | `assets/generated/twilight-long-barrel-back-atlas.png` | 352 x 198 |
...
| Twilight coherent break | Variant 2 of 4 selected as the final active crest/curl/pour/collapse silhouette | `docs/art-source/grok/twilight-breaking-wave-source.jpg` | `803b17d83dbf2b9f6c03973dfee5074b188b163d2077cb0efa2f14a2ca2f52e1` | `assets/generated/twilight-breaking-wave-atlas.png` | 512 x 136 |
...
The final coherent breaking-wave sheet came from local Grok session `019f7e05-f8fe-7bc2-ba7b-3134ed884a95`. Variant 2 of 4 was copied without modification to `docs/art-source/grok/twilight-breaking-wave-source.jpg`; the 1280 x 720 JPEG hashes to `803b17d83dbf2b9f6c03973dfee5074b188b163d2077cb0efa2f14a2ca2f52e1`. The deterministic conversion uses exterior-connected chroma removal, nearest-neighbor reduction, a 28-color palette, two-pixel packing padding, no sharpening, and an irregular alpha feather only along the lower/side continuation that joins procedural water. It produces the 512 x 136 RGBA `assets/generated/twilight-breaking-wave-atlas.png`, whose SHA-256 is `bb95f18871965a39cc414a563094e6eaec802ad2cadfac807abed764b2fd8cd0`. Its 4 x 1 cells are `curlA`, `curlB`, `pour`, and `collapse`, each 128 x 136 with anchor `[0.66, 1]`. It now supplies the active-edge poses and first generated fallback beneath the primary long-back composition; pressure cross-fades its connected crest/curl/pour poses and curl contact cross-fades into collapse.

The selected long back layer came from local Grok session `019f7e38-311e-7002-a010-188fdffd41ee`. The untouched first composition is preserved as `twilight-long-barrel-back-source.png` with SHA-256 `08cf4024ea7cefca8ab0d01b697f484e4877f094830155e61055e55b228f5b89`. A targeted two-variant edit removed the forward horizontal rails while locking the silhouette and chroma field. Variant 1 was selected as `twilight-long-barrel-back-edit-v1.png`; variant 2 remains preserved with SHA-256 `bfc8b24e90624f5ca31b71f9d29d27ff5704ff2a4f064a816459fdc43da017bc`. The deterministic build validates the selected 1280 x 720 source hash, removes exterior-connected chroma, reduces it by nearest neighbor, quantizes it to 32 colors, and applies a stepped alpha blend only where the left edge passes over restored sky. The result is the one-frame 352 x 198 RGBA `twilight-long-barrel-back-atlas.png` with SHA-256 `bd0286dcbff1fea14fb0395f0fd3cec768fc3d873049094d25b5a757af265198` and anchor `[0.48, 0.92]`.

Two local Qwen Image Edit experiments are preserved under `docs/art-source/qwen` for comparison but are not selected or loaded at runtime. The 1024 x 576 pass hashes to `69de98e4a1ea58f652b9884fa92cdc0fe485ee97a5d41a5876541eca843618ad`; it cropped the upper crest and introduced hundreds of near-magenta fringe colors. The native 1280 x 720 pass hashes to `5631965be4a4f5eee2cc0c95ced933cd8c3282d6a8a5812ebde523953fdb758e`; it preserved the frame but regenerated the exact long horizontal rails the edit was meant to remove. Both were rejected after source-size and runtime-size comparison. `tools/art/run-qwen-edit.mjs` remains as a reusable offline helper, but Qwen is not a build or browser dependency.
...
- The coherent-break atlas supplies active-edge poses and the first normal-palette fallback; the complete earlier Twilight barrel follows it and is also the authored High Contrast option. The connected procedural curl is the final fallback if those generated layers are absent.
- The long back layer is now the primary normal-palette Twilight body. It places the rider inside a broad D-shaped pocket, supplies curved face planes instead of reversible horizontal streaks, and hands the pitching lip to the coherent-break/segmented-curtain layers in front. Its stepped left alpha transition reveals actual sky as the wave passes without exposing a vertical atlas seam.
...
- The two Qwen seam experiments were useful diagnostic comparisons, but neither improved the selected composition: one damaged framing/chroma and the other recreated the objectionable rails. They remain authoring evidence only.

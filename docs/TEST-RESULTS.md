...
The current native Node run passed **352 tests with 0 failures**. `npm run check` parsed **41 JavaScript modules** successfully.
...
The canonical local-browser matrix contains 178 deterministic captures.

- The gallery, capture script, contact-sheet source, and checked-in directory contain the same **178 scene identifiers**.
- The 35-scene Kaki-Land pass covers menu/results, initial and late run, the complete five-band seam, both travel directions, cutback, Turbo, small/medium/huge air, high-air layers, The Last Relay, every Webring Relay beat, dolphin/shark, wipeout/recovery, both run modes, and access/quality variants. It completed with **0 page, runtime, console, network, or HTTP errors**.
...
- A separate ordinary Kaki-Land real-keyboard chase recorded the same reversible contract under `docs/images/qa-kaki-land-chase`: the opening break advanced from x20.71 to x108.06 with zero wipeouts, the right-going lead sent it to x-59.04, and a committed left cutback let it pursue back to x20.01.
- A fresh real-keyboard vertical-camera pass captured four complete riding/lip/launch/rise/apex/descent/re-entry/touchdown/post-landing sequences. Launch checkpoints landed at 101-102 ms and 251-252 ms. Ordinary-right held stage, HUD, horizon, rider framing, and backdrop movement at 0; ordinary-left used at most one pixel of configured impact shake. Maximum-right reached a 98.09-pixel legacy signed camera signal while actual stage/HUD/horizon offsets remained 0, held the complete rider at logical y=68, added 0 pixels of excess motion beyond physics, restored the visual coast before touchdown, and returned the backdrop state to 0 by 300 ms after landing. Reduced Motion reproduced the fixed-stage and y=68 visibility results with zero shake. Exact metrics and frame sequences are checked in under `docs/images/qa-world-camera`.

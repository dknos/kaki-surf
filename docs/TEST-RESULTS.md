...
The current native Node run passed **360 tests with 0 failures**. `npm run check` parsed **41 JavaScript modules** successfully.
...
- Simple line/Action/Trick/Turbo input and HUD contracts, Advanced Q/E/F/T, mode-switch clearing, aliases, edge buffering, gamepad selection, dead zone, independent touch pointers, and keyboard/gamepad/touch trick-manifest parity.
...
- Full pointer event to `InputManager.consumeStep()` to `SurfSimulation.update()` to completed-manifest integration for 80 ms taps, 220 ms directional holds, same-frame press/release, 250 ms pre-takeoff buffering, explicit Down + Trick stance switching, low-air fallback, high-air Varial, deterministic chains, and Simple landing auto-release.
- Advanced Q/E/F/T on-wave context, 0.35-second transient eligibility buffering, readable quick grabs, extended physical holds, discrete release continuation, ordered multi-trick manifests, repeated-edge anti-farming, one-shot expiry, rotation naming, provisional scoring, exact-repeat decay, landing bank, and wipeout loss.
...
- Four-tier launch qualification, earned Turbo-only maximum air, coherent stage-camera ownership, exact inverse panorama crop anchoring, signed horizontal panorama travel, no background fragment repaint through advanced-break cutouts, zone milestones, altitude-filtered surf/wind, and fixed-step ascent/descent continuity.
...
- Escalating monotonic curl pressure through entry, jumps, crashes, and recovery, eight-second opening grace, skilled relief, a moving safe respawn lane, deterministic idle/skilled threat windows, and collision-aligned profile-specific contact.
...
- Four canonical condition IDs derived from one registry, Kaki-Land renderer/audio identity, `signalBreak` profile installation, community world-profile resolution, additive save persistence, and safe fallback for unknown IDs.
- Deterministic Webring Relay scheduling at bounded 45–90 second intervals, isolated random streams, pooled three-gate state, reachable ordering in both directions, warning exclusion, fragment/mural/Approval completion, result snapshots, and one Signal Held Flow-protection charge.
...
- Asset-manifest paths for all 16 runtime atlas families, generated-atlas dimensions, four-second optional-image timeout/fallback behavior, preserved offline wave-source hashing and conversion, local-only static imports, 384 x 216 backing Canvas, and no runtime remote generation dependency.
...
The canonical local-browser matrix contains 182 deterministic captures.

- The gallery, capture script, contact-sheet source, and checked-in directory contain the same **182 scene identifiers**.
- The 39-scene Kaki-Land pass covers menu/results, initial and late run, the complete five-band seam, both travel directions, cutback, Turbo, small/medium/huge air, high-air layers, The Last Relay, every Webring Relay beat, dolphin/shark, wipeout/recovery, both run modes, and resident/access/quality variants. It completed with **0 page, runtime, console, network, or HTTP errors**.
- Existing aerial fixtures cover Coastal Sky, Cloud Layer, Upper Atmosphere, Kaki Space, and atmospheric re-entry across the original three conditions; Kaki-Land adds cloud, upper, space, and Last Relay reveal fixtures through the same continuous-camera contract.
...
- The responsive rerun captured menu, active play, pause, results, and options at 1280 x 720, 1920 x 1080, 1366 x 768, 1024 x 768, 390 x 844, 812 x 375, 844 x 390, and 915 x 412. Each short-landscape fixture shows the continuous stick and large independent actions around a full-height playfield; the 390 x 844 active fixture shows the landscape gate instead of portrait controls.
...
- A fresh 844 × 390 DPR2 trick-control CDP pass sent real 80 ms touches through the production input path. Simple and Advanced F each showed `QUEUED`, then `VARIAL`, then exactly one completed manifest entry and one haptic request. Simple Trick measured 68 px, Action 76 px, Advanced Q/E/F/T 52 × 52 px, and both decks had zero overlap. Blur and portrait rotation cleared held pre-takeoff requests; landscape resumed cleanly with zero console, runtime, network, or HTTP errors.
...
- Selected Grok and Vertex/Nano exploration sources, all 16 runtime atlas families, menu/results, the shared column break in every condition, Kaki-Land’s reviewed Grok panorama/decor conversion, distant race craft, wildlife, controls, banners, carrier, and access modes were inspected at actual output size. The active break is code-native and consumes canonical contact directly.
...
- A separate ordinary Kaki-Land real-keyboard chase recorded the same reversible contract under `docs/images/qa-kaki-land-chase`: the opening break advanced from x20.52 to x108.78 with zero wipeouts, the right-going lead sent it to x-58.60, and a committed left cutback let it pursue back to x20.34.
- A fresh real-keyboard vertical-camera pass captured four complete riding/lip/launch/rise/apex/descent/re-entry/touchdown/post-landing sequences. Launch checkpoints landed at 101-102 ms and 251-252 ms. Ordinary right and left held stage, HUD, horizon, rider framing, and backdrop movement at zero. Maximum-right and Reduced-Motion maximum used the same continuous stage camera, peaking at 24.42 and 24.47 logical pixels while the HUD stayed fixed; both counter-crops returned to the coast by touchdown, recorded zero ocean-anchor error, and landed cleanly from the camera lifecycle's perspective. Exact metrics and frame sequences are checked in under `docs/images/qa-world-camera`.
...
- The aerial fixtures confirm continuous coast/cloud/upper/space crops through all four 1536 x 640 condition masters, masked atmospheric entry with no exposed row or doubled coast, a vertically fixed surf stage, complete rider-only high-air framing, Reduced Motion visibility, and proactive atmospheric return without changing horizontal world position.
- A fresh-storage controller-only CDP run selected Moon Log, started Twilight, paused and resumed without leaking A/B into gameplay, opened Settings, changed Effects volume, closed with B, navigated Results, and retried with A. The refreshed 40-scene lifecycle matrix visually verified full-height short-landscape playfields with the analog deck and a dedicated 390 × 844 landscape gate; pointer and rotation behavior is covered by the native interaction tests above.
- The refreshed 40-scene responsive pass verified that the mode selector and start action remain reachable before board/condition customization, short landscape menus remain scrollable, the Endless SET panel clears the top HUD, and full-width Best Trick/Best Tube rows do not overlap either results column.

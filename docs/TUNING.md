...
During `curlGrace`, collision stays disabled while Twilight's curtain still advances at its authored 6.2 logical-pixels-per-second opening speed. After grace, it accelerates smoothly toward `curlThreatMaxSpeed`. Recent valid pumps, full carves, reversals, clean landings, and high physical speed provide bounded relief; signed rider travel never reverses the threat. Entry and wipeout presentation keep the curl moving while collision remains disabled. Recovery preserves safety by moving Kaki's entry lane ahead of the current contact, never by stopping or rewinding the curl. Twilight's visible curtain edge is welded three logical pixels behind the canonical contact and therefore advances left-to-right with the threat rather than mirroring the rider.
...
A riding Tube Tuck enters when the local zone is `critical` or risk is at least 0.58, while face position remains within 0.16..0.76. The held session uses a wider hysteresis band: risk at least 0.50 and face position 0.12..0.82, plus 0.12 seconds of pocket-exit grace to absorb fixed-step edge jitter. Holding Simple Trick or Advanced `trick4` (T) sustains the pose; Moon Log resolves the pose as Soul Arch. Tube time scales both steering axes to 46%, adds a small 8%-scaled local wave-push contribution, and scores `58 * (1 + risk * 0.55) * multiplier` style points per second plus 0.025 normalized Flow per second. Releasing exits deliberately; losing the pocket requires a release before the same held input can start another session. A ride of at least 0.45 seconds earns the clean-exit grade, and the foreground veil fades for 0.18 seconds instead of popping away.
...
`cameraResponse` is 5.8 and `juiceIntensity` is 1; both are presentation-only. Horizontal `camera.worldX` retains the forward dead zone. The legacy signed `camera.worldY` value is now only a rider-framing signal: `cameraLayerOffset("stage")` and its compatibility `"world"` alias can return impact shake but can never translate the surf stage from that signal. Routine airs use a logical board-anchor band at y=52; qualified airs at least 64 pixels high use y=68 so Kaki's upright silhouette clears the HUD. The offset remains zero until the natural path crosses its band and returns to zero continuously on descent. Reduced Motion removes shake but retains required exceptional-air visibility. Atmospheric presentation uses one authored `aerialAltitude` signal, a 0.22 enter/0.14 exit hysteresis band, fixed coast/cloud/upper/space crops, a 16-row reveal mask, and bounded 0.52/s rise and 1.05/s return rates. New saves use Simple Controls, Wave Read Assist `full`, Reduced Flash on, 70% screen shake, touch controls on, and steering/landing assists off. Saves created before `controlMode` existed migrate to Advanced so their learned mapping remains intact; fresh and malformed saves use Simple. Either steering or landing assistance multiplies the score by 0.82. Reduced Motion, Reduced Flash, High Contrast, and Wave Read modes do not change physics or scoring.

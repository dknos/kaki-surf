...
The static game includes six condition backgrounds and 17 compact generated atlas families. Twilight's active art stack uses a one-frame long-barrel back wall behind Kaki, segmented coherent curl/pour and foreground foam in front, restrained internal falling-water texture, and board-contact spray; coherent-break, earlier full-barrel, and procedural layers form the fallback chain. The remaining atlases cover classic wave progression, modular breaker pieces, dolphin, shark, whale, birds, boats, air traffic, powerups, boards, the festival carrier, and UI ornaments. Selected Grok sheets are preserved under `docs/art-source/grok`; rejected local Qwen comparisons are preserved separately for auditability. `tools/art/build-grok-assets.py` validates source hashes, removes connected chroma, extracts cells, downsamples, quantizes, blends only declared continuation edges, and rebuilds the transparent atlases under `assets/generated`.

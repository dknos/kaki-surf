...
The rider can commit to left- or right-going travel. A reversal must scrub through the existing board velocity before the new direction commits, and the surfer, wake, airborne approach, and switch-stance scoring all follow that signed direction. On Twilight's long face, Kaki crosses the screen first; only forward travel beyond a camera dead zone advances the world. The distant coast and sparse diagonal face flecks continuously move left with absolute wave travel, making forward speed readable before the camera follows while never flipping on a cutback. The board's maximum speed is a real cap, not a target that every line receives automatically.
...
- whales progress through distant, blow, breach, ramp, bidirectional ride, splash, and departure phases;

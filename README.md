...
Every condition is now staged on the same production long-break compositor. One registered wave mass supplies the outer wall, hollow barrel, falling curtain, and tapered face; its foremost foam pixel is pinned to the simulation's catch contact while the rider can traverse almost the full screen in either direction. The break travels left-to-right, internal whitewater falls downward on a pause-safe clock, the cleared area reveals the real condition sky/backwater, and neither motion mirrors when Kaki reverses. Twilight retains the deeper rideable tube and large-air camera treatment. Reduced Motion freezes the falling-water cycle and vertical camera shift without changing gameplay.
...
The static game includes six condition backgrounds and 15 compact generated atlas families. The active wave stack starts with a 384 x 216 continuous side-break texture whose black isolation field, tube opening, bottom rail, and continuation edges are deterministically keyed and blended. Live geometry owns collision, stage scale, sky reveal, falling-water cadence, impact churn, and the complete fallback; the six-cell travelling-break and board-contact families add subordinate motion detail. The remaining atlases cover classic wave studies, modular breaker pieces, dolphin, shark, whale, birds, boats, air traffic, powerups, boards, the festival carrier, and UI ornaments. Selected Grok sheets are preserved under `docs/art-source/grok`; rejected local Qwen comparisons are preserved separately for auditability. `tools/art/build-grok-assets.py` validates source hashes, extracts and keys sources, downsamples, quantizes, and rebuilds the transparent atlases under `assets/generated`.

...
The rider can commit to left- or right-going travel. A reversal must scrub through the existing board velocity before the new direction commits, and the surfer, wake, airborne approach, and switch-stance scoring all follow that signed direction. On Twilight's long face, Kaki crosses the screen first; only forward travel beyond a camera dead zone advances the world. Cutting back never drags the scenery or water marks backward. The board's maximum speed is a real cap, not a target that every line receives automatically.
...
The static game loads six condition backgrounds and 13 compact generated atlas families. The active MVP wave silhouette is code-native: collision-aligned gravity columns, fixed contrail foam, a long face, impact churn, and board-contact spray. Two rejected full-wave studies remain preserved in the 15-family offline build output for provenance, but are no longer downloaded or composited at runtime. The loaded atlases cover wave accents, dolphin, shark, whale, birds, boats, air traffic, powerups, boards, the festival carrier, and UI ornaments. Selected Grok sheets are preserved under `docs/art-source/grok`; rejected local Qwen comparisons are preserved separately for auditability. `tools/art/build-grok-assets.py` validates source hashes, extracts and keys sources, downsamples, quantizes, and rebuilds the transparent atlases under `assets/generated`.

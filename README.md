...
The static game includes six condition backgrounds and 14 compact generated atlas families. Twilight's active art stack uses a live horizon-to-trough wall and long unstriped face, a six-cell travelling-break atlas for falling-water detail, and board-contact spray. Runtime geometry owns the crest, collision edge, tube, passed-sky opening, waterfall cadence, and collapse, so missing art cannot restore the retired giant C-wave. The remaining atlases cover classic wave progression, modular breaker pieces, dolphin, shark, whale, birds, boats, air traffic, powerups, boards, the festival carrier, and UI ornaments. Selected Grok sheets are preserved under `docs/art-source/grok`; rejected local Qwen comparisons are preserved separately for auditability. `tools/art/build-grok-assets.py` validates source hashes, removes connected chroma, extracts cells, downsamples, quantizes, and rebuilds the transparent atlases under `assets/generated`.

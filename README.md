...
The native suite passes **340/340 tests** and the syntax gate checks **41 JavaScript modules**. New invariants include complete pointer-event-to-manifest trick integration, pre-takeoff and eligibility buffering, deterministic Simple fallback/chaining, quick and extended grabs, Simple landing auto-release, Advanced hold risk, anti-farming, keyboard/gamepad/touch parity, and the existing 30/60/144 Hz fixed-step equivalence. The broader suite continues to cover the persistent Auto/Full/Mobile setting, presentation-only density scaling, wave/camera ownership, traversal, Turbo, wildlife, scoring, and the fixed 384 x 216 playfield. The local gallery contains **143 deterministic 1280 x 720 captures**, including re-entry in all three conditions, while `docs/images/qa-world-camera` records real-input launch, apex, descent, touchdown, and post-landing frame sequences in both travel directions and Reduced Motion.

...
The native suite passes **360/360 tests** and the syntax gate checks **41 JavaScript modules**. Kaki-Land invariants cover canonical condition resolution, `signalBreak` installation, save migration, panorama identity and anchoring, renderer/audio identity, bidirectional travel, aerial pursuit, High Contrast fallback anchoring, deterministic Webring Relay scheduling, QA-only fixture visibility, random-stream isolation, reachable gates in both directions, warning exclusion, mural completion, Approval, and Signal Held. The broader suite continues to cover pointer-event-to-manifest trick integration, Auto/Full/Mobile presentation scaling, wave/camera ownership, Turbo, wildlife, scoring, fixed-step equivalence, and the fixed 384 x 216 playfield. The local gallery contains **182 deterministic 1280 x 720 captures**, including Kaki-Land’s complete Relay sequence, four-condition comparison, access modes, resident anchoring, and high-air reveal. `docs/images/qa-kaki-land-chase` adds a real-keyboard right-run, cutback, left return, and pursuing-break sequence.
